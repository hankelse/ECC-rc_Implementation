...
void ECC_FR::find_all_common_neighbors() {
    for (Edge* edge : G->_edges) {
        ...
        const vector<Node*>& common_neighbors = node_set_intersect(n1->neighbors, n2->neighbors);

        // Add common_neighbors to common_neighbor_sets
        common_neighbor_sets[edge->index] = common_neighbors;


        vector<Node*> neighborhood = common_neighbors;
