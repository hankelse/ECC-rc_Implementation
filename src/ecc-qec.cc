 ...
 * @return 
 */
Node* ECC_QEC::extract_node(vector<Node*>& potential_additions, Clique* clique) {
    ...
    for (Node* clique_node : clique->nodes) {
        for (Node* potential : potential_additions) {
            ...
            if (connecting_edge != nullptr && !((node_clique_assignments[clique_node->index] == node_clique_assignments[potential->index]) && (node_clique_assignments[potential->index] == clique->index))) {
